...
int bst_define_variable(bst_scope_ptr scope, ast_node_ptr ast_node) {
    ...
    if (content.type != NT_FUNC_DECL &&
        content.type != NT_GETTER &&
        content.type != NT_SETTER)
        content.value_tree->value = TT_NULL;
    else // ast_node is func
...
bst_node_ptr bst_declare_variable(bst_node_ptr var_def_node, token_type_t value) {
    bst_value_node_ptr new_value = var_def_node->content.value_tree;
    new_value->n_of_children++;

    new_value->children = realloc(var_def_node->content.value_tree->children, sizeof(bst_value_node_ptr) * new_value->n_of_children);
    ...
    new_value->children[new_value->current_index]->parent = new_value;
    new_value->children[new_value->current_index]->children = NULL;
    new_value->children[new_value->current_index]->n_of_children = 0;
    new_value->children[new_value->current_index]->current_index = -1;
    new_value->children[new_value->current_index]->value = value;

    var_def_node->content.value_tree = new_value;
    var_def_node->content.value_tree = var_def_node->content.value_tree->children[var_def_node->content.value_tree->current_index];
...
int bst_increase_scope(bst_scope_ptr *scope) {
    ...
    if ((*scope) != NULL) {
        ...
        bst_scope_ptr walk = (*scope);
        while(walk != NULL) {
            if(walk->tree) {
                if(bst_increase_var_reach(walk->tree, &walk->tree->content.value_tree))
                    return ERROR_INTERNAL;
            }
...
// increases reach of each var in current and all parent scopes
int bst_increase_var_reach(bst_node_ptr scope_tree, bst_value_node_ptr *value_tree_ptr) {
    if(scope_tree == NULL)
        return 0;

    if(bst_increase_var_reach(scope_tree->left, &scope_tree->left->content.value_tree))
        return ERROR_INTERNAL;

    if(bst_increase_var_reach(scope_tree->right, &scope_tree->right->content.value_tree))
        return ERROR_INTERNAL;



    if (scope_tree->content.type == NT_FUNC_DECL ||
        scope_tree->content.type == NT_GETTER ||
        scope_tree->content.type == NT_SETTER)
        return 0;

    if(strncmp(scope_tree->content.name, "__", 2) == 0)
        return 0;


    bst_value_node_ptr value_tree = (*value_tree_ptr);
...
void bst_decrease_scope(bst_node_ptr tree_node) {
    ...
    if (tree_node->content.type == NT_FUNC_DECL ||
        tree_node->content.type == NT_GETTER ||
        tree_node->content.type == NT_SETTER)
        return;

    if(strncmp(tree_node->content.name, "__", 2) == 0)
        return;
